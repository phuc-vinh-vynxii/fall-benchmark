"""From-scratch trainer (Case 1). Trains a randomly-initialized architecture on a small
balanced subset of MergedFallDataset for the smoke test, then scales to the CS/CV splits.

Usage:
    python train.py --model x3d                         # small-subset smoke test
    python train.py --model tsm --subset 100 --epochs 5
    python train.py --config config.yaml                # all settings from YAML
    python train.py --model x3d --train-split ../splits/cs/train.csv \\
                    --val-split ../splits/cs/val.csv     # full CS split, no subset

Run from inside the train_from_scratch/ folder.
"""
...
DEFAULTS = {
    ...
    "img_size": 224,
    "batch_size": 2,
    "lr": 1e-4,                # higher LR for from-scratch
    "weight_decay": 0.05,
    "epochs": 5,
    "subset_per_class": 100,   # small subset for smoke test; set null for full split
...
def main():
    ...
    print(f"[config] {cfg}")
    run(cfg, build_model_fn=build_model, pretrained=False)
